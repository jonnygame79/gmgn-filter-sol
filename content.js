...
(function () {
    ...
    function init() {
        ...
        if (!window.location.href.includes('gmgn.ai/sol/address/')) {
            return;
        }
    ...
    function applyFilterToRow(tokenAddress) {
        ...
        if (token_age == null || token_age == undefined) {
            ...
            if (info.launchpad && platformImages[info.launchpad]) {
                ...
                // Set border color and width based on migration block time
                const borderColor = info.migration_block_time > 0 ? 'rgba(255, 230, 9, 0.8)' : 'rgba(255, 255, 255, 0)';
                const borderWidth = info.migration_block_time > 0 ? '2px' : '0x';
    ...
    function startGetTokenInfo() {
        setInterval(async () => {
            try {
                ...
                tokenIndex++;
                const tokenDetail = await fetch(`https://gmgn.ai/api/v1/mutil_window_token_security_launchpad/sol/${tokenAddress}?device_id=${device_id}&client_id=${client_id}&from_app=${from_app}&app_ver=${app_ver}&tz_name=${tz_name}&tz_offset=${tz_offset}&app_lang=${app_lang}&fp_did=${fp_did}&os=${os}`);
                ...
                let cursor = ""
                while (1) {
                    const activites = await fetch(`https://gmgn.ai/vas/api/v1/wallet_activity/sol?type=buy&type=sell&device_id=${device_id}&client_id=${client_id}&from_app=${from_app}&app_ver=${app_ver}&tz_name=${tz_name}&tz_offset=${tz_offset}&app_lang=${app_lang}&fp_did=${fp_did}&os=${os}&wallet=${wallet}&token=${tokenAddress}&limit=50&cursor=${cursor}`);
                ...
                let first_buy_amount = lastTx.cost_usd / (lastTx.gas_usd / lastTx.gas_native);
                ...
                const rug_info = await fetch(`https://gmgn.ai/api/v1/mutil_window_token_link_rug_vote/sol/${tokenAddress}?device_id=${device_id}&client_id=${client_id}&from_app=${from_app}&app_ver=${app_ver}&tz_name=${tz_name}&tz_offset=${tz_offset}&app_lang=${app_lang}&fp_did=${fp_did}&os=${os}`, {
                    method: 'GET',
...
https://gmgn.ai/api/v1/token_mcap_candles/sol/${tokenAddress}?device_id=${device_id}&client_id=${client_id}&from_app=gmgn&app_ver=${app_ver}&tz_name=${tz_name}&tz_offset=${tz_offset}&app_lang=en-US&fp_did=${fp_did}&os=web&resolution=1m&from=${(first_tx_time - first_tx_time % 60) * 1000}&to=${first_tx_time * 1000 + 4 * 3600000}&limit=240`)
...
https://gmgn.ai/api/v1/token_mcap_candles/sol/${tokenAddress}?device_id=${device_id}&client_id=${client_id}&from_app=gmgn&app_ver=${app_ver}&tz_name=${tz_name}&tz_offset=${tz_offset}&app_lang=en-US&fp_did=${fp_did}&os=web&resolution=1s&from=${first_tx_time * 1000 + 1000}&to=${first_sell_all_time * 1000}&limit=240`)
    ...
    function startTableRowMonitoring() {
        ...
        const observer = new MutationObserver(function (mutations) {
            if (!window.location.href.startsWith("https://gmgn.ai/sol/address")) {
    ...
    function getCurrentFilters() {
        const savedFilters = localStorage.getItem('gmgn-filters');
        if (savedFilters) {
            try {
    ...
    function openMultipleBrowser() {
        ...
        if (filtered_tokens.length > 0) {
            while (opened_browsers < filtered_tokens.length) {
                ...
                window.open(`https://gmgn.ai/sol/token/${filtered_tokens[opened_browsers]}?tag=All&maker=${wallet}&filter=All`, '_blank');
                opened_browsers++;
                cnt++;
                if (cnt == 7)
                    break;
            ...
            while (opened_browsers < tokens.length) {
                ...
                window.open(`https://gmgn.ai/sol/token/${tokens[opened_browsers]}?tag=All&maker=${wallet}&filter=All`, '_blank');
                opened_browsers++;
                cnt++;
                if (cnt == 7)
                    break;
            ...
            <div class="gmgn-filter-modal">                
                <div class="gmgn-filter-content" id="draggable">        
                    ...
                    <div class="gmgn-filter-section">                    
                        ...
                        <div class="gmgn-filter-checkboxes">
                            <label><input launchpad type="checkbox" value="Pump.fun">Pump</label>                            
                            <label><input launchpad type="checkbox" value="ray_launchpad"> Launchlab</label>              
                            <label><input launchpad type="checkbox" value="meteora_virtual_curve"> Meteora</label>        
                            <label><input launchpad type="checkbox" value="Moonshot"> Moonit</label>                   
                            <label><input launchpad type="checkbox" value="heaven">Heaven</label>
                            <label><input migrated type="checkbox" value="pump-swap">P_AMM</label>
                            <label><input migrated type="checkbox" value="damm">M_DAMM</label>
                            <label><input migrated type="checkbox" value="cpmm">R_CPMM</label>                            
                            <label><input migrated type="checkbox" value="heaven-swap">HEAVEN_SWAP</label>
                        </div>
                    </div>
    ...
    function applyFilters() {
        ...
        localStorage.setItem('gmgn-filters', JSON.stringify(filtersData));
    ...
    function clearFilters() {
        ...
        // Clear localStorage
        localStorage.removeItem('gmgn-filters');
